...
public class SgepService {
	...
	public Funcionario autenticaLogin(LoginDTO login) {		
		Long count = funcionarioRepo.autenticaLogin(login.getMatricula(), login.getSenha());	
		
		if(count > 0)
			return funcionarioRepo.recuperaFuncionarioPorLogin(login.getMatricula(), login.getSenha());
